 ...
 */
GENTICS.Aloha.Repositories.wpInternalLinks = new GENTICS.Aloha.Repository('wpInternalLinks');
 ...
 * @cfg
 */
GENTICS.Aloha.Repositories.wpInternalLinks.settings.data = [
//	{ name: 'Aloha Editor - The HTML5 Editor', url:'http://aloha-editor.com', type:'website' },
//	{ name: 'Aloha Logo', url:'http://www.aloha-editor.com/images/aloha-editor-logo.png', type:'image'  }
];
 ...
 * @hide
 */
GENTICS.Aloha.Repositories.wpInternalLinks.folder =[];

/**
 * initalize wpInternalLinks, parse all links, build folder structure and add 
 * additional properties to the items
 */
GENTICS.Aloha.Repositories.wpInternalLinks.init = function() {
	
	var that = this;
	// Load all post information from wordpress backend
	jQuery.ajax({
		url: FrontEndEditor.data.ajax_url,
		type: 'POST',
		async: true,
		dataType: 'json',
		data: 'action=front-end-editor-link-query&nonce=' + FrontEndEditor.data.nonce, //+ '&query=' + p.queryString, //  + '&' + postData,
		success: function(response){
			that.settings.data = response.results;
    ...
    // repository name
    this.repositoryName = 'wpInternalLinks';
};

GENTICS.Aloha.Repositories.wpInternalLinks.addFolder = function (path, name) {
 ...
 */
GENTICS.Aloha.Repositories.wpInternalLinks.query = function( p, callback) {
 ...
 */
GENTICS.Aloha.Repositories.wpInternalLinks.getChildren = function( p, callback) {
	var d = [];
...
GENTICS.Aloha.Repositories.wpInternalLinks.parseUri = function(str) {
	var	o = {
			strictMode: false,
 ...
 */
GENTICS.Aloha.Repositories.wpInternalLinks.getObjectById = function (itemId, callback) {
	var d = [];
